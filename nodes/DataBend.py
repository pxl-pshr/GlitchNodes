...
class DataBend:
    ...
    def apply_color_shift(self, image, params):
        ...
        if params["channel_shift_mode"] == "rgb_split":
            ...
            shifts = np.random.randint(-int(params["color_intensity"] * 30), int(params["color_intensity"] * 30), 3)
            for c in range(3):
    ...
    def apply_distortions(self, image, params):
        ...
        if params["wave_distortion"] > 0:
            ...
            for c in range(3):
                result[..., c] = ndimage.map_coordinates(image[..., c], [y, x_displaced], order=1, mode="reflect")
    ...
    def generate_databend(
        ...
        print(f"\n{'=' * 50}")
        print("Applying DataBend effects:")
