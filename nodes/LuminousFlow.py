...
class LuminousFlow:
    ...
    def enhance_colors(self, color, vibrancy, contrast):
        ...
        enhanced_color = np.where(enhanced_color > 0.5, enhanced_color * 1.3, enhanced_color * 0.7)
        ...
        with tqdm(total=len(line_positions), desc="Drawing lines", unit="line") as pbar:
            for pos in line_positions:
                ...
                for i in range(len(points) - 1):
                    self.draw_line(
                        canvas, points[i], points[i + 1], colors[i], params["line_thickness"], params["glow_spread"]
                    )
