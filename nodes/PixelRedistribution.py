...
class PixelRedistribution:
    ...
    def process_single_image(self, image, color_size, order, distance_mode, pattern, strength, contrast, brightness, invert):
        device = image.device
        ...
        coords = torch.stack(torch.meshgrid(torch.arange(height, device=device),
        ...
        for start_key in start_keys:
            ...
            transform_colorspace[start_key] = [
                tuple(points[i].cpu().numpy()) for i in sorted_indices
            ]
            ...
            while queue and len(end_keys) < len(transform_colorspace[start_key]) - 1:
                ...
                adj_colors = self.get_adjacent_colors(
                    current_key_tensor, searched, order, color_size
                )
    ...
    def redistribute_pixels(self, image, color_size, order, distance_mode, pattern, strength, contrast, brightness, invert):
        try:
            ...
            print(f"Error in PixelRedistribution: {str(e)}")
            raise e